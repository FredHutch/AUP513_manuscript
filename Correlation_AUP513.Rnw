...
ggplot(dat, aes(x=response, y=ics.pctpos_adj, colour=ics.tcellsub)) + theme_bw() +
  ...
  theme(#plot.margin = unit(c(1,1,0,1), "cm"),
    ...
    strip.text.y = element_text(size=15)) + scale_y_sqrt() 

corsAll <- rbind(corsCD4, corsCD8)
corsAll$Group <- "Group 5"
corsAll$Assay <- "BAMA"
@
...
corsDat <- rbind(corsCD4, corsCD8)
corsDat$Group <- "Group 5"
...
ggplot(dat, aes(x=response, y=ics.pctpos_adj, colour=ics.tcellsub)) + theme_bw() +
  ...
  theme(#plot.margin = unit(c(1,1,0,1), "cm"),
    ...
    axis.text.y = element_text(size=10),
    strip.text.x = element_text(size=15),
    strip.text.y = element_text(size=15))  + scale_y_sqrt() + scale_x_sqrt()
...
\begin{figure}[H]
\begin{center}
<<corrplot_Grp5,fig.width=15,fig.height=10, echo=FALSE, warning=FALSE, cache=FALSE>>=
# CD4 and CD8
dat <- corsAll[corsAll$Group=="Group 5",]

ggplot(dat, aes(x=cor, y=Cytokine.Antigen, colour=ics.tcellsub, shape=Assay)) + theme_bw() + geom_point(size=5, alpha=0.6) +
  scale_colour_manual(name="T-cell population", values=c("CD4"="red", "CD8"="blue")) +
  xlab("Correlation") +
  ...
  theme(#plot.margin = unit(c(1,1,0,1), "cm"),
    ...
    axis.text.y = element_text(size=10)) + geom_vline(xintercept = 0, linetype = "longdash")
@
\caption{Plot showing correlation between T-cell ICS assay and B-cell assays for each cytokine antigen combination colored by CD4/CD8 T-cell populations in Group 5}
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
<<corrplot_Grp6,fig.width=15,fig.height=10, echo=FALSE, warning=FALSE, cache=FALSE>>=
# CD4 and CD8
dat <- corsAll[corsAll$Group=="Group 6",]
...
ggplot(dat, aes(x=cor, y=Cytokine.Antigen, colour=ics.tcellsub, shape=Assay)) + theme_bw() + geom_point(size=5, alpha=0.6) +
  ...
  theme(#plot.margin = unit(c(1,1,0,1), "cm"),
    ...
    axis.text.y = element_text(size=10)) + geom_vline(xintercept = 0, linetype = "longdash")
@
\caption{Plot showing correlation between T-cell ICS assay and B-cell assays for each cytokine antigen combination colored by CD4/CD8 T-cell populations in Group 6}
\end{center}
\end{figure}
