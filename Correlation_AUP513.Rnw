...
corsAll <- corsAll[order(-corsAll$cor),]
@
...
ggplot(dat, aes(x=cor, y=reorder(Cytokine.Antigen, cor, median), colour=Assay)) + theme_bw() + geom_point(size=5, alpha=0.6) +
  scale_colour_manual(name="T-cell population", values=c("BAMA"="red", "ADCC GTL"="blue", "NAb"="green")) +
  xlab("Correlation") +
  ylab("Cytokine Antigen Combination") +
  facet_wrap(~ics.tcellsub,ncol=1) +
  theme(#plot.margin = unit(c(1,1,0,1), "cm"),
    legend.position="top",
    legend.title=element_text(size=12,vjust=-1.6),
    legend.text=element_text(size=12,vjust=-1.6),
