...
datTot$ics.group <- mapvalues(datTot$ics.groupOLD, from = c("5", "6"), to = c("Group 5", "Group 6"))
@
% {\scriptsize
% \tab \textbf{Footnote: \newline}
...
corsGrp5 <- ddply(dat[dat$ics.group=="Group 5",], c("ics.antigen","ics.cytokine","ics.group"), summarise, cor.test = 
...
corsGrp6 <- ddply(dat[dat$ics.group=="Group 6",], c("ics.antigen","ics.cytokine","ics.group"), summarise, cor.test = 
                       paste(round(cor.test(ics.pctpos_adj, response, method="spearm", alternative="g")$estimate[[1]], 2), ", pval = ",
                              round(cor.test(ics.pctpos_adj, response, method="spearm", alternative="g")$p.value, 2)))
...
ggplot(dat, aes(x=response, y=ics.pctpos_adj, colour=ics.group)) + theme_bw() +
  ...
  geom_point() +    
  scale_colour_manual(name="Group", values=c("Group 5"="red", "Group 6"="blue")) +
...
# grp5 <-subset(dat, ics.group=="Group 5")
...
# 
# 
# grp6 <-subset(dat, ics.group=="Group 6")
